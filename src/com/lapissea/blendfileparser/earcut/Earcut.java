package com.lapissea.blendfileparser.earcut;
...
public final class Earcut{
	 ...
	 */
	public static TIntList earcut(double[] data){
	 ...
	 * @return List containing groups of three vertice indices in the resulting array forms a triangle.
	 */
	public static TIntList earcut(double[] data, int dim){
	 ...
	 * @return List containing groups of three vertice indices in the resulting array forms a triangle.
	 */
	public static TIntList earcut(double[] data, int[] holeIndices, int dim){
		...
		TIntList triangles=new TIntArrayList();
	...
	private static void earcutLinked(Node ear, TIntList triangles, int dim, double minX, double minY, double size, int pass){
		if(ear==null)
			return;
	...
	private static void splitEarcut(Node start, TIntList triangles, int dim, double minX, double minY, double size){
	...
	private static Node cureLocalIntersections(Node start, TIntList triangles, int dim){
		Node p=start;
		do{
	...
	private static boolean isEarHashed(Node ear, double minX, double minY, double size){
		...
		double minTX=a.x<ear.x?(a.x<c.x?a.x:c.x):(ear.x<c.x?ear.x:c.x), minTY=a.y<ear.y?(a.y<c.y?a.y:c.y):(ear.y<c.y?ear.y:c.y),
			maxTX=a.x>ear.x?(a.x>c.x?a.x:c.x):(ear.x>c.x?ear.x:c.x), maxTY=a.y>ear.y?(a.y>c.y?a.y:c.y):(ear.y>c.y?ear.y:c.y);
